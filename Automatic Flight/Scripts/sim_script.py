...
def smooth_func(spd):
    global spd_prev
    if spd == 0:spd_smth = (spd * 0.00005)+(spd_prev*0.99995)
...
#?  5m  spd = 0.3 0.0005, 0.000000, 0.000000
#?  7m  spd = 0.5 0.0007, 0.000001, 0.000000
#?  10m spd = 0.9 0.0010, 0.000002, 0.000001

#?  5m  spd = 0.9 0.0005, 0.000000, 0.000000
#?  10m spd = 0.3 0.0010, 0.000002, 0.000001
...
out = cv2.VideoWriter(os.path.join('Record','output'+str(seq)+'.avi'),fourcc, 20.0,  (1280,1800))
...
while not rospy.is_shutdown():
    try:
        ...
        #img = client.get_screen_frame()data = rospy.wait_for_message("/webcam/image_raw", Image)
            ...
            try:
                ...
                if return_confirmation == 0 and rotate == 0:
                    if point == 1:
                        ...
                        velo = smooth_func(100)
                        if speed >0:send_local_velocity(velo*max_speed,speed, 0,speed*(1-max_speed))
                        elif speed <0:send_local_velocity(velo*max_speed,speed,  0,speed*(1-max_speed))
                        #print(speed*0.1)
                    elif point == 3 :     
